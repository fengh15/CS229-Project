...
def fE2(hand, table, index, move, possibleHand, oppHasPassed):
    ...
    if table.AgentCardCount[index]==18 and move is not None:
        feats.append(((2, 'Start with', move[0]), 1))
    if counterCount(possibleHand, move) == 0:
        feats.append(((2, 'No Counters'), 1))
    if move is not None and oppHasPassed[tuple(move[1])] != 0 and move[0] >= oppHasPassed[tuple(move[1])]:
        feats.append(((2, 'Has passed', tuple(move[1])), 1))
    plusMove(hand, move)
    return feats
